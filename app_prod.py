...
# --- CONFIGURAÇÃO ---
def get_env(key, default): return os.environ.get(key, default)
...
CFG = {
    ...
    "telegram_admin_ids": [s.strip() for s in get_env("TELEGRAM_ADMIN_ID", "8084292904").split(",")],
...
def db_stats(p_login=None):
    ...
    p_f = {**f, "status": "pago"}
    p = col_cobrancas.count_documents(p_f)
    res = list(col_cobrancas.aggregate([{"$match": p_f}, {"$group": {"_id": None, "total": {"$sum": "$valor"}}}]))
...
async def gerar_pix_web(req: PixReq, c: HTTPBasicCredentials = Depends(security)):
    ...
    payload = {
        ...
        "client": {"name": req.nome_pagador, "email": "cliente@email.com", "phone": "11999999999", "document": "12345678909"}
    }
...
async def gerar_pix_jogo_api(req: PixReq, c: HTTPBasicCredentials = Depends(security)):
    try:
        ...
        return {"success": False, "message": f"Robô não configurado: {e}"}
...
# --- TELEGRAM BOT (Mínimo para não travar) ---
